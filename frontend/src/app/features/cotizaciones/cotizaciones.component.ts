...
import { CotizacionResumen, DashboardCotizaciones, EstadoCotizacion } from '../../core/models/models';
import { CotizacionService } from '../../core/services/cotizacion.service';
import { AuthService } from '../../core/services/auth.service';
import { MonedaPipe } from '../../core/pipes/moneda.pipe';
import { ESTADOS, ETIQUETAS_ESTADO, TAGS_ESTADO } from './estado-cotizacion';
...
export class CotizacionesComponent implements OnInit, OnDestroy {
  ...
  desde = '';
  hasta = '';
  estado: EstadoCotizacion | null = null;
  busqueda = '';

  dashboard: DashboardCotizaciones | null = null;
  ...
  private cargarDashboard(): void {
    this.cotizacionService.dashboard(this.desde, this.hasta).subscribe({
      next: (dashboard) => {
      ...
      error: (err) => {
        this.error = err?.error?.error ?? 'No se pudieron cargar las métricas de cotizaciones.';
      },
    });
  }
  ...
  private cargarListado(): void {
    ...
    this.cotizacionService
      ...
      .subscribe({
        ...
        error: (err) => {
          this.error = err?.error?.error ?? 'No se pudieron cargar las cotizaciones.';
          this.cargando = false;
        },
      });
  }

  private construirOpciones(dashboard: DashboardCotizaciones): Highcharts.Options {
    const estilo = getComputedStyle(document.documentElement);
    const colorTexto = estilo.getPropertyValue('--text-muted').trim() || '#6b7280';
    const colorPorEstado: Record<EstadoCotizacion, string> = {
      BORRADOR: estilo.getPropertyValue('--gray-400').trim() || '#9ca3af',
      ENVIADA: estilo.getPropertyValue('--info-base').trim() || '#2563eb',
      ACEPTADA: estilo.getPropertyValue('--success-base').trim() || '#16a34a',
      RECHAZADA: estilo.getPropertyValue('--error-base').trim() || '#dc2626',
      VENCIDA: estilo.getPropertyValue('--warning-base').trim() || '#d97706',
