...
class LibroVentasExcelServiceTest {
    ...
    void generaUnExcelReleiblesConLasFilasYSubtotalesCorrectos() throws IOException {
        ...
        try (XSSFWorkbook workbook = new XSSFWorkbook(new ByteArrayInputStream(excel))) {
            ...
            // Fila 1: blank. Fila 2: encabezado subtotales (Tipo, Cantidad, Neto Afecto,
            // Neto Exento, IVA, Total). Filas 3-4: subtotales. Fila 5: total.
            Row encabezadoSubtotales = hoja.getRow(2);
            assertEquals("Neto Afecto", encabezadoSubtotales.getCell(2).getStringCellValue());
            assertEquals("Neto Exento", encabezadoSubtotales.getCell(3).getStringCellValue());

            Row subtotalFactura = hoja.getRow(3);
            assertEquals("Factura", subtotalFactura.getCell(0).getStringCellValue());
            assertEquals(1000.00, subtotalFactura.getCell(2).getNumericCellValue(), 0.001);
